...
export class StudentService {
  ...
  async getDashboardSummary(user?: RequestUser) {
    ...
    const assignmentViews = await this.getAssignmentViews(
      studentId,
      user?.email,
    );
    const now = Date.now();
  ...
  private buildShortFeedbackSummary(grade?: GradeDocument | null) {
    ...
    return [
      missingCount
        ? `Missing ${missingCount} expected classes or members.`
        : null,
      relationshipCount
  ...
  private buildStudentFeedback(grade?: GradeDocument | null) {
    ...
    return {
      ...
      strengthsDetected: (grade?.rubricBreakdown ?? [])
        .filter(
          (item) => item.awardedMarks >= item.maxMarks && item.maxMarks > 0,
        )
        .map((item) => `${item.label} matched the expected structure`)
        .slice(0, 3),
      missingComponents: missingComponents.map((item) => item.message),
      incorrectRelationships: incorrectRelationships.map(
        (item) => item.message,
      ),
